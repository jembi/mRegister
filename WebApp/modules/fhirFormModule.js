export class FHIRFormHandler {
    ...
    constructor(containerId, parentUrl, childConfig = [], fhirServerUrl) {
      ...
      this.childConfig = childConfig || []; // array of { url, linkId }
    ...
    async init() {
      try {
        ...
        // Load child questionnaires in parallel
        ...
        // Insert child questionnaires into correct placeholders
    ...
    transformToFHIR(itemsData) {
      ...
      function process(item) {
        ...
        if (val && val.code && val.system) {
          ans.answer.push({ valueCoding: { code: val.code, system: val.system, display: val.text } });
